...
</style><link rel="stylesheet" href="assets/apple.css?v=11">
...
function sigmaTokenLimpio($valor){
  $valor=trim((string)$valor);
  // Admite pegar el valor solo, "Bearer ..." o la cabecera completa "Authorization: Bearer ...".
  $valor=preg_replace('/^Authorization\s*:\s*/i','',$valor);
  $valor=preg_replace('/^Bearer\s+/i','',$valor);
  return trim((string)$valor);
}
function sigmaTokenActual($pdo){
  try{
    $token=(string)$pdo->query("SELECT token FROM configuracion_sigma WHERE id=1 LIMIT 1")->fetchColumn();
    $token=sigmaTokenLimpio($token);
    if($token!=='') return $token;
  }catch(Throwable $e){}
  return sigmaTokenLimpio((string)getenv('SIGMA_API_TOKEN'));
}
function sigmaBaseUrl(){
  $base=trim((string)getenv('SIGMA_API_URL'));
  return $base!=='' ? $base : 'https://mdprime.sigma.st/api/customers';
}
...
if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  try{
    ...
    if($a==='add_normal'){
      ...
      if($cad&&$cad<$today)$estado='Inactivo';
      if($nombre!==''){
        // Un usuario que ya pertenece a un referente nunca debe crearse como cliente normal.
        $dupRef=$pdo->prepare("SELECT id FROM referidos WHERE LOWER(TRIM(nombre))=LOWER(TRIM(?)) LIMIT 1");
        $dupRef->execute([$nombre]);
        if($dupRef->fetch()) throw new Exception('Ese usuario ya pertenece a un referente y no puede añadirse como cliente normal: '.$nombre);
    ...
    if($a==='save_sigma_token'){
      $token=sigmaTokenLimpio($_POST['sigma_token']??'');
      if($token==='') throw new Exception('Pega el token de Sigma antes de guardar.');
      if(strlen($token)<20) throw new Exception('El token parece incompleto. Cópialo entero desde F12.');
      $st=$pdo->prepare("INSERT INTO configuracion_sigma(id,token) VALUES(1,?) ON DUPLICATE KEY UPDATE token=VALUES(token), actualizado_en=CURRENT_TIMESTAMP");
      $st->execute([$token]);
      $msg='Token de Sigma guardado correctamente en Railway.';
    }
    if($a==='clear_sigma_token'){
      $pdo->exec("UPDATE configuracion_sigma SET token=NULL WHERE id=1");
      $msg='Token guardado eliminado. El panel usará el token de Render si todavía existe.';
    }
    if($a==='test_sigma'){
      $token=sigmaTokenActual($pdo);
      if($token==='') throw new Exception('No hay ningún token de Sigma configurado.');
      $base=sigmaBaseUrl();
      $sep=str_contains($base,'?')?'&':'?';
      $json=sigmaGetJson($base.$sep.http_build_query(['page'=>1,'perPage'=>1]),$token);
      [$rows,$last]=sigmaRowsAndLastPage($json);
      $msg='Conexión con Sigma correcta · Token válido · API responde correctamente.';
    }
    if($a==='import_sigma'){
      $token=sigmaTokenActual($pdo);
      if($token==='') throw new Exception('Falta configurar el token de Sigma. Guárdalo en Configuración Sigma o en Render.');
      $base=sigmaBaseUrl();
      ...
      try{
        ...
        // Prioridad absoluta: REFERIDOS -> CLIENTES NORMALES.
        // No usamos UNION, así evitamos cualquier conflicto de collations.
        $findRef=$pdo->prepare("SELECT id,fecha_caducidad,estado FROM referidos WHERE LOWER(TRIM(nombre))=LOWER(TRIM(?)) LIMIT 1");
        $updateRefs=$pdo->prepare("UPDATE referidos SET fecha_caducidad=?,estado=? WHERE LOWER(TRIM(nombre))=LOWER(TRIM(?))");
        $deleteNormalByName=$pdo->prepare("DELETE FROM clientes_normales WHERE LOWER(TRIM(nombre))=LOWER(TRIM(?))");

        $findNormal=$pdo->prepare("SELECT id,fecha_caducidad,estado FROM clientes_normales WHERE LOWER(TRIM(nombre))=LOWER(TRIM(?)) LIMIT 1");
        $updateNormal=$pdo->prepare("UPDATE clientes_normales SET fecha_caducidad=?,estado=? WHERE id=?");
        $insertNormal=$pdo->prepare("INSERT INTO clientes_normales(nombre,contacto,telefono,telegram,fecha_alta,fecha_caducidad,estado,nota) VALUES(?,'','','',?,?,?,'')");

        $nuevos=0;$actualizados=0;$sinCambios=0;$referidosActualizados=0;$normalesEliminados=0;
        foreach($usuariosSigma as $u){
          $estado=($u['caduca']<$today)?'Inactivo':'Activo';

          // 1) Si existe como referido, se actualiza allí y jamás se crea como normal.
          $findRef->execute([$u['nombre']]);
          $ref=$findRef->fetch();
          if($ref){
            if(($ref['fecha_caducidad']??null)!==$u['caduca'] || ($ref['estado']??'')!==$estado){
              $updateRefs->execute([$u['caduca'],$estado,$u['nombre']]);
              $referidosActualizados += $updateRefs->rowCount();
            }else{
              $sinCambios++;
            }

            // Limpieza preventiva de cualquier copia histórica en clientes_normales.
            $deleteNormalByName->execute([$u['nombre']]);
            $normalesEliminados += $deleteNormalByName->rowCount();
            continue;
          }

          // 2) Solo cuando no existe como referido se gestiona como cliente normal.
          $findNormal->execute([$u['nombre']]);
          $ex=$findNormal->fetch();
          if($ex){
            if(($ex['fecha_caducidad']??null)===$u['caduca'] && ($ex['estado']??'')===$estado){$sinCambios++;continue;}
            $updateNormal->execute([$u['caduca'],$estado,$ex['id']]);
            $actualizados++;
          }else{
            $insertNormal->execute([$u['nombre'],$today,$u['caduca'],$estado]);
            $nuevos++;
          }
        }
        $pdo->commit();
        $msg='Sigma importado correctamente · Referidos actualizados: '.$referidosActualizados.' · Normales eliminados por duplicidad: '.$normalesEliminados.' · Normales nuevos: '.$nuevos.' · Normales actualizados: '.$actualizados.' · Sin cambios: '.$sinCambios.' · Backup Railway #'.$backupId.' · Total Sigma: '.count($usuariosSigma);
      }catch(Throwable $e){
...
$sigmaTokenDb=sigmaTokenLimpio($sigmaCfg['token']??'');
$sigmaTokenEnv=sigmaTokenLimpio((string)getenv('SIGMA_API_TOKEN'));
$sigmaTokenDisponible=($sigmaTokenDb!=='' || $sigmaTokenEnv!=='');
$sigmaTokenOrigen=$sigmaTokenDb!=='' ? 'Railway' : ($sigmaTokenEnv!=='' ? 'Render' : 'Sin configurar');

/* ===== TOTALES GENERALES DEL PANEL ===== */
$totalGestionado = $totalClientes + $totalRefs + $totalNormales;
$totalActivosGlobal = $totalActivos + $totalNormalesActivos;
$totalInactivosGlobal = $totalInactivos + $totalNormalesInactivos;
$pctActGlobal = ($totalActivosGlobal + $totalInactivosGlobal) > 0
...
$totalPagNormales = max(1, (int)ceil($totalNormales / $normalesPorPagina));
if($paginaNormales > $totalPagNormales) $paginaNormales = $totalPagNormales;
$clientesNormalesPagina = array_slice(
  $clientesNormales,
  ($paginaNormales - 1) * $normalesPorPagina,
  $normalesPorPagina
);
...
<link rel="stylesheet" href="assets/apple.css?v=11">
...
<script>window.MDPRIME_METRICS=<?=json_encode([
  'referentes'=>$totalClientes,
  'referidos'=>$totalRefs,
...
<script src="assets/apple-app.js?v=11" defer></script>
</body></html>
